            ...
            try
            {
                ...
                GlobalVariables.InvokeIfRequired(this, () =>
                {
                    barStaticItemStatus.Caption = $"{DateTime.Now.ToString("yyyy/MM/dd HH:mm")} " +
                              ...
                              $"- Delay Print = {GlobalVariables.DelayPrintInterval} ms";
                            ...
                            for (int i = 5; i < _rowUsed + 5; i++)
                            {
                                ...
                                if (!string.IsNullOrEmpty(_row[$"A{i}"].Value.TextValue))
                                {
                                    coreData.Add(new tblCoreDataCodeItemSize()
                                    {
                                        ...
                                        PlasticBag1Qty = _row[$"X{i}"].Value.NumericValue,
                                        PlasticBag2Qty = _row[$"Y{i}"].Value.NumericValue,
                                        WrapSheetQty = _row[$"Z{i}"].Value.NumericValue,
                                        FoamSheetQty = _row[$"AA{i}"].Value.NumericValue,
                                        PlasticBag1Weight = _row[$"AF{i}"].Value.NumericValue,
                                        PlasticBag2Weight    = _row[$"AG{i}"].Value.NumericValue,
