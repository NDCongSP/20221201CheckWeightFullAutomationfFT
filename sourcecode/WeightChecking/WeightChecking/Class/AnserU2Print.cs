...
namespace WeightChecking
{
    ...
    /// Kết nối qua TCP (thay thế COM port), tự động reconnect khi mất kết nối.
    ...
    public class AnserU2TcpDriver : IDisposable
    {
        ...
        private async Task ConnectAndReceiveLoopAsync(CancellationToken ct)
        {
            while (!ct.IsCancellationRequested)
            {
                try
                {
                    ...
                    await connectTask; // propagate any exception from ConnectAsync
        ...
        private async Task ReceiveLoopAsync(CancellationToken ct)
        {
            ...
            var frameBuffer = new System.Collections.Generic.List<byte>(64);
            bool inFrame = false;
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                catch { break; } // connection reset → reconnect
                ...
                for (int i = 0; i < read; i++)
                {
                    ...
                    if (b == 0x02) // STX — bắt đầu frame mới
                    {
                        frameBuffer.Clear();
                        frameBuffer.Add(b);
                        inFrame = true;
                    }
                    else if (b == 0x03 && inFrame) // ETX — kết thúc frame
                    {
                        frameBuffer.Add(b);
        ...
        public void Write(byte[] data)
        {
            ...
            {
                // lỗi ghi — ReceiveLoop sẽ phát hiện và kích hoạt reconnect
            }
            finally
            {
                _writeLock.Release();
